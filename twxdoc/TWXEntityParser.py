...
class ThingworxJSONEntityParser:
    def __init__(self, filename, rootfolder, **kw):
        ...
        #store each key with file mapping.
        self.top_file_list = {}
    ...
    def export(self):
        ...
        with open(self.sourcefile, 'r') as json_data:
            # print("reading ....")
            # objects = ijson.items(json_data,'Things.Thing.item')
            # print("Read done, parsing...")
            # for obj in objects:
            #     print('{}:{}'.format(stopcounter, obj))
            #     stopcounter += 1
            #     if stopcounter > 100:
            #         return
            self.root = ijson.parse(json_data)
            event_type_list = []
            for prefix, event, value in self.root:
                if not event in event_type_list:
                    event_type_list.append(event)

                # if (not prefix.startswith('ApplicationKeys')) and (not prefix==''):
                #     continue
